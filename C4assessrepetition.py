...
print(f'1\t\t{annual_income:9,.0f}\t\t{savings_contr:8,.0f}\t\t\t{total_savings + savings_contr + (total_savings * .06):10,.0f}')

# total_savings variable is redefined
total_savings = total_savings + savings_contr + (total_savings * .06)
...
for year in range(2, difference):
    ...
    print(f'{year}\t\t{annual_income:9,.0f}\t\t{savings_contr:8,.0f}\t\t\t{total_savings:10,.0f}')
