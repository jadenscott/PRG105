...
class LookGUI:
    def __init__(self, master):
        ...
        # TODO add three buttons to the bottom frame: *DONE*
        ...
        # pack bottom frame
        # TODO pack the buttons you just created *DONE*
    ...
    def search_first(self):
        # TODO 1. get the data from the search_entry box *DONE*
...
class AddGUI:
    def __init__(self, master):
        ...
        # widgets for top frame - label and entry box for name
        # TODO add labels and entry boxes for each of the following data fields: *DONE*
        ...
        # TODO pack each of the labels and entry boxes you just created *DONE*
        ...
        # middle frame - label for results
        # TODO create a StringVar() called self.message *DONE*
        # TODO create a Label in self.result_frame with width of 45 that will display the contents of self.message
        self.message = tkinter.StringVar()
        self.result_label = tkinter.Label(self.result_frame, textvariable=self.message,
                                          width=45)

        # pack results into frame
        # TODO pack the Label you just created *DONE*
        self.result_label.pack(side='left')

        # buttons for button frame
        # TODO add two buttons into the button_frame *DONE*
        ...
        self.add_button = tkinter.Button(self.button_frame, text='Add',
                                         command=self.add_customer)
        self.main_menu_button = tkinter.Button(self.button_frame, text='Main Menu',
                                               command=self.add.destroy)

        # pack button frame
        # TODO pack the buttons you just added *DONE*
    ...
    def add_customer(self):
        # TODO get data from the entry widgets for first name, last name, phone and email; strip off white space *DONE*
        first_name = self.first_name_entry.get().strip()
        last_name = self.last_name_entry.get().strip()
        phone = self.phone_entry.get().strip()
        email = self.email_entry.get().strip()
        # check to make sure data is available to add, if so, add to database
        # TODO use an if statement to verify that none of the fields are empty *DONE*
        if (len(self.first_name_entry.get()) != 0 and len(self.last_name_entry.get()) != 0 and
                len(self.phone_entry.get()) != 0 and len(self.email_entry.get()) != 0):
            customer_db = None
            try:
                # TODO use the try-except statement to connect to the database and INSERT the data *DONE*
                # TODO remember to commit your changes
                customer_db = sqlite3.connect('customers.db')
                cur = customer_db.cursor()
                cur.execute('''INSERT INTO Customers (FirstName, LastName, Phone, Email) VALUES (?, ?, ?, ?)
                ''', (first_name, last_name, phone, email))
                customer_db.commit()
            except sqlite3.Error as err:
                # TODO if there is an SQLite error, set an error message into self.message *DONE*
                ...
                if customer_db is not None:
                    # TODO 1. set self.message to say "Customer added." *DONE*
                    ...
                    self.message.set('Customer added.')
                    ...
                    customer_db.close()
                # TODO aligned to the try block, set the focus to the first name entry widget *DONE*
                # TODO     to set focus, use <field>.focus_set()
                self.first_name_entry.focus_set()
        else:
            # TODO set self.message to indicate data was not added due to incomplete data *DONE*
            self.message.set('Complete all fields.')
...
class ChangeGUI:
    def __init__(self, master):
        ...
        # TODO 1. add a statement for each of these four entry widgets to change the state to DISABLED *DONE*
        # TODO      Example:  self.first_name_entry['state'] = tkinter.DISABLED
        self.first_name_entry['state'] = tkinter.DISABLED
        self.last_name_entry['state'] = tkinter.DISABLED
        self.phone_entry['state'] = tkinter.DISABLED
        self.email_entry['state'] = tkinter.DISABLED
        ...
        # buttons for button frame
        # TODO 1. add three buttons to the button_frame for Update, Cancel and Main Menu *DONE*
        ...
        self.update_button = tkinter.Button(self.button_frame, text='Update',
                                            command=self.change_customer, state=tkinter.DISABLED)
        self.cancel_button = tkinter.Button(self.button_frame, text='Cancel',
                                            command=self.cancel_update, state=tkinter.DISABLED)
        self.main_menu_button = tkinter.Button(self.button_frame, text='Main Menu',
                                               command=self.change.destroy)
    ...
    def change_customer(self):
        # TODO get data from each of the entry widgets into variables, strip off white space *DONE*
    ...
    def swap_status(self):
        ...
        self.phone_entry['state'] = tkinter.DISABLED
        self.email_entry['state'] = tkinter.DISABLED
        self.update_button['state'] = tkinter.DISABLED
    ...
    def find_customer(self):
        ...
        if len(result) == 1:
            # TODO enable the first name, last name, phone and email entry widgets *DONE*
            ...
            self.first_name_entry['state'] = tkinter.NORMAL
            self.last_name_entry['state'] = tkinter.NORMAL
            self.phone_entry['state'] = tkinter.NORMAL
            self.email_entry['state'] = tkinter.NORMAL
            self.first_name_entry.insert(0, result[0][1])
            self.last_name_entry.insert(0, result[0][2])
            self.phone_entry.insert(0, result[0][3])
            self.email_entry.insert(0, result[0][4])
            # swap button states to allow for Update or Cancel,
            # but disallow searching for a new ID while a record is active
            # TODO 1. Enable the Update and Cancel buttons using: <widget>['state'] = tkinter.ACTIVE *DONE*
            ...
            self.update_button['state'] = tkinter.ACTIVE
            ...
            # TODO 1. set self.message to indicate that the Customer ID was not found *DONE*
...
def find_customer_by_name(name, first_name):
    ...
    try:
        ...
        if first_name is True:
            cur.execute('SELECT * FROM Customers WHERE FirstName LIKE ?', (pattern_to_match,))
        else:
            cur.execute('SELECT * FROM Customers WHERE LastName LIKE ?', (pattern_to_match,))
